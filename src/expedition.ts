...
export function expeditionMarkup(s: State) {
  return (
    ...
    roles
      ...
      .join("") +
    `<h3 class="list-heading">AUTOMATION</h3><div class="legacy-explainer"><strong>${automationUnlocked(s) ? "Orbital logistics online" : "Unlock with your first ascension"}</strong><p>While the game is visible, buy up to 10 structures each second. The budget caps spending from the balance at the start of each tick. No offline purchases.</p><label class="setting-row">Auto-buy structures<input id="auto-enabled" type="checkbox" ${s.autoEnabled ? "checked" : ""} ${automationUnlocked(s) ? "" : "disabled"}></label><label class="setting-row">Priority<select id="auto-policy"><option value="efficient" ${s.autoPolicy === "efficient" ? "selected" : ""}>Best base output per cost</option><option value="cheapest" ${s.autoPolicy === "cheapest" ? "selected" : ""}>Cheapest first</option></select></label><label class="setting-row">Spend at most per tick<select id="auto-budget">${[10, 25, 50].map((n) => `<option value="${n}" ${s.autoBudget === n ? "selected" : ""}>${n}% of reserve</option>`).join("")}</select></label></div><h3 class="list-heading">ARTIFACT LOADOUT · ${s.artifacts.length}/3</h3>` +
    artifacts
      .map(
        (a, i) =>
